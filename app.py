...
if uploaded_file is None:
    render_sidebar(0, 0)
    st.info(
        "Upload a medical image (.dcm, .png, .jpg) from the sidebar to get started."
    )
    st.stop()
...
def _apply_operation(img, op, par, dev):
    ...
    if op == "Gaussian Blur":
        ...
        Filters.difference_of_gaussian(
            img,
            out,
            low_sigma=par["low_sigma"],
            high_sigma=par["high_sigma"],
            device=dev,
        )
        ...
        algo = BreastMaskAlgorithm(mask_only=True, device=dev)
        algo(img, out)
    elif op == "Apply Breast Mask":
        algo = BreastMaskAlgorithm(mask_only=False, device=dev)
        algo(img, out)
        ...
        algo = BitDepthNormAlgorithm(
            bits_stored=bits, target_max=par["target_max"], device=dev
        )
        algo(img, out)
    elif op == "Top-Hat":
        algo = TopHatAlgorithm(radius=par["radius"], device=dev)
        algo(img, out)
    elif op == "K-Means":
        algo = KMeansAlgorithm(
            k=par["k"], max_iter=par["max_iter"], tol=par["tol"], device=dev
        )
        algo(img, out)
    elif op == "FCM":
        algo = FCMAlgorithm(
            c=par["c"], m=par["m"], max_iter=par["max_iter"], tol=par["tol"], device=dev
        )
        algo(img, out)
    elif op == "PFCM":
        algo = PFCMAlgorithm(
        ...
        algo = FebdsAlgorithm(method=par["method"], device=dev)
        algo(img, out)
...
def _show_metrics(elapsed, device, algo):
    ...
    if torch.cuda.is_available() and device != "cpu":
        m3.metric(
            "Peak GPU Memory", f"{torch.cuda.max_memory_allocated() / 1024**2:.1f} MB"
        )
    else:
...
if roi_config is not None:
    ...
    def _apply_on_roi(image, operation, params, device, roi_config):
        ...
        if roi_config.get("normalize"):
            RegionOfInterest.normalize(
                roi_img, divisor=roi_config.get("normalize_divisor", 4095.0)
            )
    ...
    with col_proc:
        st.markdown("**Processed**")
        overlay = overlay_roi_result(
            orig_display, result_display, x_min, y_min, x_max, y_max
        )
